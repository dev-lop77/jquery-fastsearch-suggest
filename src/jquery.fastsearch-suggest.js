...
(function ($) {
  ...
  var defaults = {
    // Data sources
    url:            null,   // AJAX endpoint URL; query sent as ?{param}={typed}
    param:          'q',    // Query parameter name for AJAX calls
    data:           null,   // Pre-loaded string array for local filtering
    dataUrl:        null,   // URL to fetch a JSON array once at init, then filter locally

    // Behaviour
    minChars:       2,      // Minimum characters before triggering suggestions
    maxSuggestions: 5,      // Maximum items shown in the dropdown
    debounce:       300,    // AJAX debounce delay in milliseconds
    matchLastWord:  false,  // When true, suggest on the last space-separated word only

    // Callbacks
    onSelect:       null    // function(value) called when user selects a suggestion
  };
  ...
  // token  — the word being completed (last word when matchLastWord, else full input)
  // match  — the suggested completion for token
  // fullInput — the full current input value (equals token when matchLastWord is false)
  function _showGhost($input, token, match, fullInput) {
    ...
    if (!$ghost.length) {
      // Copy input metrics once so ghost text aligns with input text exactly.
    ...
    // Transparent span covers the full input text; gray span shows only the suffix.
    // $ghost.text() therefore returns prefix + token + suffix = full final value.
    $ghost.empty()
      .append($('<span>').text(fullInput).css('color', 'transparent'))
  ...
  function _suggest($input, suggestions, token, settings, $wrapper, fullInput) {
    ...
    // Always show ghost text for the top match.
    _showGhost($input, token, suggestions[0], fullInput);

    // Show dropdown only when there are 2 or more matches.
  ...
  $.fn.fastsearchSuggest = function (method, options) {
    ...
    // Support calling as $(el).fastsearchSuggest(options) — no method string.
    ...
    return this.each(function () {
      ...
      if ($input.data('fastsearchSuggest')) { return; } // Already initialised.

      var instanceId   = 'fss-' + Date.now() + '-' + Math.floor(Math.random() * 1000);
      var localData    = null; // Populated by dataUrl fetch or inline data option.
      var pendingInput = null; // Stores last {token, fullInput} typed before dataUrl loaded.

      // Priority: data > dataUrl > url (AJAX).
      if (settings.data) {
